...
class PandAGUI(tk.Tk):
    ...
    def add_profile_tab(self, event):
        ...
        if self.notebook.select() == self.notebook.tabs()[-1]:
            ...
            self.delete_profile_button = ttk.Button(new_profile_tab,
                                                    text ="Delete Profile",
                                                    command = self.delete_profile_tab)
            self.delete_profile_button.grid(column = 7,
                                            row = 10,
                                            padx = 5,
                                            pady = 5,
                                            columnspan=1,
    ...
    def delete_profile_tab(self):
        ...
        answer = tk.messagebox.askyesno("Close Profile",
                                        "Delete this profile? Are you sure?")
        ...
        for n,tab in enumerate(self.notebook.tabs()[0:-1]): #noqa
            ...
            ttk.Label(proftab_object, text ='Profile '+str(n)).grid(column =0,
                                                                    row = 0,
                                                                    padx = 5,
                                                                    pady = 5,
                                                                    sticky="w")
    ...
    def load_config(self):
        ...
        answer = tk.messagebox.askyesno("Close/Open New", "Finished editing this profile? Continue?") #noqa
    ...
    def save_config(self):
        ...
        panda_config_yaml = fd.asksaveasfile(mode='w',
    ...
    def build_exp_run_frame(self):
        ...
        self.get_plans_button = ttk.Button(self.run_frame,
                                           text ="Get Plans",
                                           command=self.get_plans).grid(column = 2,
                                                                        row = 1,
                                                                        padx = 5,
                                                                        pady=5,
                                                                        columnspan=1,
        ...
        self.get_devices_button = ttk.Button(self.run_frame,
                                             text ="Get Devices",
                                             command = self.get_devices).grid(column=2,
                                                                              row=3,
                                                                              padx=5,
                                                                              pady=5,
                                                                              columnspan=1,
        ...
        self.stop_plans_button = ttk.Button(self.run_frame,
                                            text ="Stop Plan",
                                            command = self.stop_plans).grid(column=2,
                                                                            row=5,
                                                                            padx=5,
                                                                            pady=5,
                                                                            columnspan=1,
        ...
        self.pause_plans_button = ttk.Button(self.run_frame,
                                             text ="Pause Plan",
                                             command = self.pause_plans).grid(column=2,
                                                                              row=7,
                                                                              padx=5,
                                                                              pady=5,
                                                                              columnspan=1,
        ...
        self.resume_plans_button = ttk.Button(self.run_frame,
                                              text ="Resume Plan",
                                              command = self.resume_plans).grid(column=2, #noqa
                                                                                row =9,
                                                                                padx=5,
                                                                                pady=5,
                                                                                columnspan=1,
        ...
        self.run_plan_button = ttk.Button(self.run_frame,
                                          text ="Run Plan",
                                          command = self.run_plan).grid(column=2,
                                                                        row=11,
                                                                        padx=5,
                                                                        pady=5,
                                                                        columnspan=1,
    ...
    def build_global_settings_frame(self):
        ...
        self.global_settings_frame = ttk.Frame(self.window,
                                               borderwidth=5,
                                               relief='raised')
        ...
        self.load_button = ttk.Button(self.global_settings_frame,
                                      text ="Load",
                                      command = self.load_config)

        self.save_button = ttk.Button(self.global_settings_frame,
                                      text ="Save",
                                      command = self.save_config)

        self.configure_button = ttk.Button(self.global_settings_frame,
                                           text ="Upload to PandA",
                                           command = self.configure_panda)

        self.show_wiring_config_button = ttk.Button(self.global_settings_frame,
                                                    text ="Wiring config",
                                                    command=self.show_wiring_config)

        self.Opentextbutton = ttk.Button(self.global_settings_frame,
    ...
    def build_exp_info_frame(self):
        ...
        self.experiment_settings_frame = ttk.Frame(self.window,
                                                   borderwidth=5,
                                                   relief='raised')

        self.experiment_settings_frame.pack(fill ="both",
                                            expand=True,
                                            side="bottom",
                                            anchor="w")
        ...
        ttk.Label(self.experiment_settings_frame,
            text ="Instrument: "+self.configuration.instrument.upper()).grid(column=0,
                                                                             row=0,
                                                                             padx=5,
                                                                             pady=5,
                                                                             sticky="w")

        ttk.Label(self.experiment_settings_frame,text="Experiment:").grid(column=0,
                                                                          row=1,
                                                                          padx=5,
                                                                          pady=5,
                                                                          sticky="w")

        tk.Entry(self.experiment_settings_frame,
                 bd =1,
                 textvariable=self.experiment_var).grid(column=1,
                                                        row=1,
                                                        padx=5,
                                                        pady=5,
                                                        sticky="w")
    ...
    def build_active_detectors_frame(self):
        ...
        for pulse in range(PULSEBLOCKS):
            ...
            active_detectors_frame_n = ttk.Frame(self.pulse_frame,
                                                 borderwidth=5,
                                                 relief='raised')

            active_detectors_frame_n.pack(fill ="both",
                                          expand=True,
                                          side="left",
                                          anchor="w")


            Pulselabel = ttk.Label(active_detectors_frame_n,
                                   text =f"Pulse Block: {pulse+1}")
            ...
            for n, det in enumerate(PULSE_CONNECTIONS[pulse+1]):
                ...
                if (det.lower() == "fs") or ("shutter" in det.lower()):
                    ad_entry = tk.Checkbutton(active_detectors_frame_n,
                                              bd =1,
                                              text=det,
                                              state='disabled')
                    ad_entry.select()
    ...
    def __init__(self,panda_config_yaml=None):
        ...
        if user not in  ["akz63626","rjcd"]: #check if I am runing this
            ...
            try:
                ...
                answer = tk.messagebox.askyesno("PandA not Connected","PandA is not connected, if you continue things will not work. Continue?") #noqa
                if answer:
                    pass
                else:
                    quit()
        ...
        self.default_config_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),"profile_yamls","default_panda_config.yaml") #noqa
        ...
        for i in range(self.configuration.n_profiles):
            ...
            tab_names = self.notebook.tabs()
            proftab_object = self.notebook.nametowidget(tab_names[i])
            self.delete_profile_button = ttk.Button(proftab_object,
                                                    text ="Delete Profile",
                                                    command = self.delete_profile_tab)
