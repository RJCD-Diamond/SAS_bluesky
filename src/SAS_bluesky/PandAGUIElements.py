    ...
    def onDoubleClick(self, event):
        ...
        #row 1 is the group name and should just be group-n and increments for each new one #noqa
            ...
            if not PULSEBLOCKASENTRYBOX:
                self.Popup = CheckButtonPopup(self,
                                              rowid,
                                              int(column[1:])-1,
                                              x=x,
                                              y=y,
            ...
            if PULSEBLOCKASENTRYBOX:
                self.Popup = EntryPopup(self,
                                        rowid,
                                        int(column[1:])-1,
                                        text,
                                        entrytype=list)
                self.Popup.place(x=x,
                                 y=y+pady,
                                 width=width,
                                 height=height,
                                 anchor='w')
...
class CheckButtonPopup(ttk.Checkbutton):
    def __init__(self, parent, rowid, column, x,y, columns, **kw):
        ...
        self.save_pulse_button = ttk.Button(self.root,
                                            text ="Ok",
                                            command=self.on_return).grid(column=PULSEBLOCKS,
                                                                         row=0,
                                                                         padx=5,
                                                                         pady=5,
                                                                         columnspan=1,
...
class ProfileTab(ttk.Frame):
    ...
    def build_profile_tree(self):
        ...
        if not hasattr(self, 'profile_config_tree'):
            self.profile_config_tree = EditableTableview(self,
        ...
        for i, col in enumerate(COLUMN_NAMES):
            ...
            self.profile_config_tree.column(i,
                                            minwidth=widths[i],
                                            width=widths[i],
                                            stretch=True,
                                            anchor="w")
        ...
        self.profile_config_tree.grid(column=0,
                                      row=table_row,
                                      padx=5,
                                      pady=5,
        ...
        verscrlbar.grid(column=len(widths),
                        row=table_row,
                        padx=0,
                        pady=0,
                        columnspan=1,
    ...
    def generate_info_boxes(self):
        ...
        try:
            ...
            self.total_frames_label.config(text=f"Total Frames: {self.profile.total_frames}") #noqa
            self.total_time_per_cycle.config(text=f"Time/cycle: {self.profile.duration_per_cycle:.3f} s") #noqa
            self.total_time_label.config(text=f"Total time: {self.profile.duration_per_cycle*self.profile.cycles:.3f} s") #noqa

        except Exception:

            #### total frames
            self.total_frames_label = ttk.Label(self, text=f"Total Frames: {self.profile.total_frames}") #noqa
            self.total_frames_label.grid(column = 8, row = 1, padx = 5,pady = 5 ,sticky="e" ) #noqa

            self.total_time_per_cycle = ttk.Label(self, text=f"Time/cycle: {self.profile.duration_per_cycle:.3f} s") #noqa
            self.total_time_per_cycle.grid(column = 8, row = 2, padx = 5,pady = 5 ,sticky="e" ) #noqa

            ### total time

            self.total_time_label = ttk.Label(self, text=f"Total time: {self.profile.duration_per_cycle*self.profile.cycles:.3f} s") #noqa
    ...
    def build_multiplier_choices(self):
        ...
        ttk.Label(self, text ="Multipliers:").grid(column=2,
                                                   row=0,
                                                   padx=5,
                                                   pady=5,
                                                   sticky="news")
        ...
        for i in range(PULSEBLOCKS): #4 pulse blocks
            ...
            ttk.Label(self, text =f"{PULSE_BLOCK_NAMES[i]}:").grid(column=col_pos,
                                                                   row=0,
                                                                   padx=5,
                                                                   pady=5,
                                                                   sticky="nsw")
            ...
            tk.Entry(self,
                     bd =1,
                     width=10,
                     textvariable=self.multiplier_var).grid(column=col_pos,
                                                            row=0,
                                                            padx=5,
                                                            pady=5,
                                                            sticky="nes")
    ...
    def __init__(self, parent, notebook, configuration, n_profile):
        ...
        ttk.Label(self, text ="Seq Trigger").grid(column=0,
                                                  row=0,
                                                  padx=5,
                                                  pady=5,
                                                  sticky="e")
        ...
        ttk.OptionMenu(self,
                       ...
                       *self.seq_triggers).grid(column=1,
                                                row = 0,
                                                padx = 5,
                                                pady = 5,
                                                sticky="w")
        ...
        ttk.Label(self, text="No. of cycles").grid(column=0,
                                                   row=1,
                                                   padx=5,
                                                   pady=5,
                                                   sticky="e")
        ...
        ############profile settings
        self.plot_profile_button = ttk.Button(self,
                                              text="Plot Profile",
                                              command = self.commit_and_plot)

        self.insertrow_button = ttk.Button(self,
                                           text ="Insert group",
                                           command = self.insert_group_button_action)

        self.deleterow_button = ttk.Button(self,
                                           text="Delete group",
                                           command=self.delete_group_button_action)

        self.appendrow_button = ttk.Button(self,
                                           text="Add group",
                                           command=self.append_group_button_action)

        self.deletefinalrow_button = ttk.Button(self,
                                                text="Discard group",
                                                command=self.delete_last_groups_button_action)

        self.print_profile_button = ttk.Button(self,
                                               text="Print Profile",
                                               command=self.print_profile_button_action)

        self.plot_profile_button.grid(column=8,
                                      row=0,
                                      padx=5,
                                      pady=5,
                                      columnspan=1,
        ...
        self.insertrow_button.grid(column=0,
                                   row=10,
                                   padx=5,
                                   pady=5,
                                   columnspan=1,
        ...
        self.deleterow_button.grid(column=1,
                                   row = 10,
                                   padx=5,
                                   pady=5,
                                   columnspan=1,
        ...
        self.appendrow_button.grid(column=3,
                                   row = 10,
                                   padx=5,
                                   pady=5,
                                   columnspan=1,
        ...
        self.deletefinalrow_button.grid(column=4,
                                         row=10,
                                         padx=5,
                                         pady=5,
                                         columnspan=1,
        ...
        self.print_profile_button.grid(column=3,
                                       row=1,
                                       padx=5,
                                       pady=5,
                                       columnspan=1,
